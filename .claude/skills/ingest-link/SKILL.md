...
Ingest an external URL as a knowledge source: fetch and analyse its content, record it in
`knowledge/links.md`, identify which template documents need updating based on the new insights,
patch those documents, and update the template structure if the changes affect the overall layout.
...
3. Read the content and extract:
   ...
   - **Summary**: a single paragraph synthesising the source's main argument or content
...
### Phase 2 — Record in knowledge/links.md
...
- **Key insights**:
  ...
  - <insight 2>
  - …
- **Template documents updated**: <list files changed, or "none" if no changes warranted>
```
...
### Phase 3 — Identify Template Documents to Update
...
5. Produce a **patch plan**: a list of `(file, change description)` pairs for files that need updating.
   If the patch plan is empty, record "none" in the `knowledge/links.md` entry and stop here.

---

### Phase 4 — Patch Template Documents
...
### Phase 5 — Update Template Structure if Needed

Check whether the patches from Phase 4 created structural changes that require updating the
...
### Phase 6 — Update knowledge/links.md Entry

Go back to the entry written in Phase 2 and fill in the final value of
